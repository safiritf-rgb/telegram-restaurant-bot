...
# -------------------------------
# 🔴 Настройка токена
# Попытка взять токен из переменной окружения (Render / другие серверы)
TOKEN = os.environ.get("TOKEN")

# Если переменная окружения пуста (например, локально), вставляем токен прямо
if not TOKEN:
    TOKEN = "8336389852:AAGuMNPiMxdUnCO8P4CsnXDBSGzgTK_Om-w"  # 🔴 Замените на токен вашего бота для локального теста

# Админские ID
ADMINS = [973547064]  # 🔴 Замените на свой ID
...
async def save_date(update, context):
    ...
    if not validate_date(text):
        await update.message.reply_text("❌ Неверный формат. Введите дату *ДД.MM.ГГГГ*", parse_mode="Markdown")
        return
...
# -------------------------------
#  Уведомление админа (фикс KeyError)
# -------------------------------
async def notify_admin(update, context):
    ...
    if not all(k in data for k in ("date", "time", "persons")):
        await update.message.reply_text(
            "⚠️ Бронирование не завершено. Пожалуйста, заполните все поля.",
            reply_markup=booking_menu()
        )
        return
